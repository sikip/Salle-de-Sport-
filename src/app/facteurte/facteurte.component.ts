...
@Component({
  selector: 'app-facteurte',
  templateUrl: './facteurte.component.html',
  styleUrls: ['./facteurte.component.css']
})
export class FacteurteComponent implements OnInit {
  @ViewChild('contentToConvert') contentToConvert!: ElementRef;// Utilisation du point d'exclamation pour indiquer qu'il ne sera jamais nul
  ...
  downloadPDF() {
    ...
    html2canvas(content).then(canvas => {
      // Récupère la hauteur et la largeur du contenu
      const contentWidth = 350;
      const contentHeight = canvas.height;
  
      // Crée un objet JSPDF
      const pdf = new jspdf.jsPDF('p', 'px', [contentWidth, contentHeight]);
  
      // Ajoute un rectangle blanc en arrière-plan
      pdf.rect(0, 0, contentWidth, contentHeight, 'F'); // 'F' pour remplir
  
      // Convertit le canvas en image de type PNG
