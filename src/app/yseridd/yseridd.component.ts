...
@Component({
  selector: 'app-yseridd',
  templateUrl: './yseridd.component.html',
  styleUrls: ['./yseridd.component.css']
})
export class YseriddComponent implements OnInit  {
  salle: any[] = [];
  users: User[] = [];
  ...
  constructor(    private route: ActivatedRoute , private  gaeageService: GaeageserviceService,
    ...
    ngOnInit(): void {
      ...
      // Vérifiez si userId est défini avant d'appeler getUserById
      if (this.userId) {
        this.getUserById(this.userId);
      }
    }
    ...
    getUserById2(id: number): void {
      ...
      this.gaeageService.getUserById(id).subscribe(
        (data) => {
          this.userInfo2 = data;
          // Vérifie si userInfo2.image est vide
          if (!this.userInfo2.image) {
            // Si userInfo2.image est vide, attribuez-lui le chemin de l'image par défaut
    ...
    this.route.paramMap.subscribe(params => {
      ...
      if (id) {
        this.getClassesByUserId(id);
        this.countClassesByUserId(id);
        this.getUserAttributes(id);
        this.getNumberOfEventsByUserId(id);
      }
    });
  
  }

  getNumberOfEventsByUserId(id: number) {
    this.gaeageService.getNumberOfEventsByUserId(id).subscribe(
      count => {
  ...
  getUserAttributes(id: number): void {
    this.gaeageService.getUserAttributes(id).subscribe(
      (data: any) => {
  ...
  countClassesByUserId(id:number ) {
    this.gaeageService.countClassesByUserId(id)
    .subscribe(count => {
      this.numberOfClasses = count;
    });
  }
  getClassesByUserId(id: number): void {
    this.gaeageService.getClassesByUserId(id)
      .subscribe(classes => {
