...
export class FrendComponent implements OnInit {
  ...
  pageSize: number = 15; // Nombre d'éléments par page
  ...
  getAllUsersAndCoaches(): void {
    this.gaeageService.getAllUsersAndCoaches().subscribe(usersAndCoaches => {
  ...
  calculateTotalPages() {
    this.totalPages = Math.ceil(this.usersAndCoaches.length / this.pageSize);
  }
  ...
  getClassesForCurrentPage() {
    ...
    return this.usersAndCoaches.slice(startIndex, startIndex + this.pageSize);
  }
